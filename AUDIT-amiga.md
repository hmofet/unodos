...
> **✅ FIXED for raise + create (`41c05e6`)** — both make a window topmost, so the only visible deltas are that window on top + the previously-topmost losing its active title stripes; the static desktop and lower windows are unchanged and nothing is newly exposed. `raise_window`/`win_create` now call `redraw_top2` (draw second-from-top, then topmost over it) instead of `repaint_all`. Byte-identical via the new deterministic renderer `uae/render.py` (`build.sh test` = creates, `build.sh test RAISE` = creates + raises). **close still uses `repaint_all`** — it must reveal the content the removed window occluded, which needs a clipped damage-rect repaint (windows sit at arbitrary pixel-x, so that means pixel-column clipping of `fill_rect`/`draw_char`/`draw_icon16`); left as follow-up now that the renderer can verify it.

...
> **✅ FIXED — raise + create (`41c05e6`), close (`fc75a2b`).** raise/create make a window topmost, so the only deltas are that window on top + the previously-topmost losing its active stripes → `raise_window`/`win_create` call `redraw_top2` (draw second-from-top, then topmost over it), no `repaint_all`. close must reveal what the removed window occluded, but a close only changes the closed window's rows (+ the new topmost's, which restyles active), so `close_window` clips `repaint_all` to that **row band**: a `clip_y0/clip_y1` window (default = full screen) added to `fill_rect` (clamp y/h) and `draw_char`/`draw_icon16` (draw only in-band rows, precomputed skip/count), plus `clear_band`. Row-band, so no pixel-column masking. All three paths now skip the full-scene repaint. Byte-identical via the deterministic renderer `uae/render.py` — `build.sh test` (create) / `test RAISE` / `test CLOSE`, each == a full-repaint baseline; and the clip is a verified no-op at full screen (`test` == pre-clip `41c05e6`).

...
entry:
    ...
    ; Save boot drive (passed from stage2 in DL)
    mov [boot_drive], dl

    ; Detect and report memory (silent - no text output)
    call detect_memory

    ; Set up graphics mode and show welcome screen
    call setup_graphics
...
.right_border:
    ...
    ; Draw version "v3.2.0" in smaller text (4x6 font)
...
; ============================================================================
; RAM Info Display (top right corner)
; ============================================================================
...
draw_ram_info:
    ...
    ; boot sector (1) + stage2 (2KB) + kernel (16KB) + stack (~1KB) = ~20KB
    mov ax, 20
    mov word [draw_x], 290
    call draw_number_small

...
static int ahci_probe(uno_device *d)
{
    if (!uno_pc64_detached()) return 0;         /* firmware still owns it */
    g_node = d;
    return 1;
}
static const uno_match ahci_match[] = {
    { UNO_MATCH_PCI_CLASS, 0, 0, 0x01, 0x06, 0, 0 },   /* mass storage / SATA */
    { UNO_MATCH_END,       0, 0, 0,    0,    0, 0 }
};
static const uno_driver ahci_drv = {
    "ahci", UNO_BUS_PCI, UNO_DEVMGR_API, ahci_match, ahci_probe, 0
};
UNO_DRIVER(ahci_drv);

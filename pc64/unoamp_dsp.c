...
{
    int A     = amp_q20(millibel / 2);      /* sqrt of the linear gain        */
    int w0    = (int)(((long long)TWOPI_Q20 * fc) / (rate ? rate : 48000));
    int cw    = cos_q20(w0);
    int sw    = sin_q20(w0);
    int alpha = sw / 2;                     /* Q = 1.0                        */
    int aA    = A ? (int)(((long long)alpha << Q) / A) : alpha;
    int alA   = mulq(alpha, A);
    int a0    = ONE + aA;
    if (!a0) a0 = 1;
    /* Normalise by a0 as we go: keeping a0 around would cost a divide per
     * sample, and this is cut once per slider move. */
    b->b0 = (int)(((long long)(ONE + alA) << Q) / a0);
    b->b1 = (int)(((long long)(-2 * (long long)cw) << Q) / a0);
    b->b2 = (int)(((long long)(ONE - alA) << Q) / a0);
    b->a1 = b->b1;                          /* same numerator as b1 here      */
    b->a2 = (int)(((long long)(ONE - aA) << Q) / a0);
}
...
static int eq_modify(short *samples, int nframes, int nch, int rate)
{
    ...
    for (i = 0; i < nframes; i++)
        for (c = 0; c < nch && c < 2; c++) {
            int x = samples[i * nch + c] << (Q - 8);   /* headroom, not full Q */
            int y = mulq(x, g_pre_q20);
            for (b = 0; b < NB; b++) {
                biquad *q = &g_bq[b];
                int out = mulq(q->b0, y) + mulq(q->b1, q->x1[c])
                        + mulq(q->b2, q->x2[c])
                        - mulq(q->a1, q->y1[c]) - mulq(q->a2, q->y2[c]);

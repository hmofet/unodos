...
| s04 | Office: Files shows the staged corpus docs; UnoWord opens + scrolls `fmt.doc`; UnoCalc opens `formulas.xls`, clicks the `=1+2*3` cell; UnoShow opens `small.ppt` | Corpus staged to `DOCS\` on the ESP pre-boot **and** `put` onto the RAM volume at runtime: the shared Open dialog lists a volume's ROOT only and starts on volume 0, so `DOCS\` is unreachable from it (uofile.c). There are no file associations - Files opens `.UNO` only - so each app opens its own document. UnoShow has no working Ctrl-O; its File > Open is clicked. |

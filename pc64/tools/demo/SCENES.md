...
| s08 | Duum: launched ON CAMERA from its own Start-menu row, then ~26 s of walking and turning through E1M1 | ~34 s (was 13.6 s - it is the best moment in the cut and it was over before it registered). DUUM.PY's own step sizes are small (MOVE = 12 map units, TURN = 0.20 rad per press), which is why the press counts look large. Guard: no WAD in `pc64/wads/` = clean no-op with a log line. Never downloads. **Records AFTER s07/s09** - see the `py` verb note below. **The pre-launch went on 2026-08-19**: it existed because a PYAPP had no Start-menu row and the only honest launch was the call Files makes on a double-click. Duum now carries an app descriptor, so it has a row and a desktop icon, and the launch is a shot instead of a workaround. The row is found with `menu_index("duum")` against the app list the live machine reported at boot - never a baked count, which is the mistake that films the app next door under the right name. |
...
## Two recordings at once will clobber each other (2026-08-19)

`remote_qemu.py` names the boot disk **`/tmp/remote_disk.img`**, a fixed path,
and `build_disk` rewrites it at the start of every session. Two sessions
recording on the same host therefore build over each other's disk, and the one
that loses does not fail - it boots the OTHER tree and films it.

That is not hypothetical. A run from this tree read a disk another session had
just built from a checkout with no `DUUM.DESC`, found no `duum` row, and the
failure looked exactly like a bug in the shell's app registry. The arithmetic
is what settled it: their `DUUM.PY` is 140,227 bytes, `+ 48` for the container
header is 140,275, and 140,275 is what `mdir` reported on the disk - with no
`DUUM.QOI` beside it, because that tree has no descriptor to name one.

So: before recording, check nobody else is (`pgrep -af scenes.py`, and look at
the mtime on `/tmp/remote_disk.img`). If two sessions genuinely have to record
on one host, give each its own `RQ.DISK`/`RQ.DISK2` - they are module
constants, not arguments, which is the thing to fix if this happens twice.

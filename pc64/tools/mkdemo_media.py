...
static FILE *g_f; static long g_sz;
long uno_fs_size(int v, const char *n){ (void)v;(void)n; return g_sz; }
long uno_fs_read_at(int v, const char *n, long off, unsigned char *b, long m)
{ (void)v;(void)n; if (off>=g_sz) return 0; if (fseek(g_f,off,SEEK_SET)) return -1;
...
int main(int c, char**v){
  uno_media_info info; short buf[4096*2]; FILE*o; long tot=0; int n;
  if(c<3) return 2;
  g_f=fopen(v[1],"rb"); if(!g_f){perror(v[1]);return 2;}
  fseek(g_f,0,SEEK_END); g_sz=ftell(g_f); rewind(g_f);
  if(!uno_media_open(0,v[1],&info)){fprintf(stderr,"no decoder\n");return 1;}
  ...
  while((n=uno_media_decode(buf,4096))>0){
...
''')
    subprocess.run(["gcc", "-O2", "-I.", "-o", exe, src,
                    "pc64_media.c", "dec_wav.c", "dec_midi.c", "dec_mp3.c", "-lm"], check=True)
    return exe
...
Every file was generated by pc64/tools/mkdemo_media.py:
  ...
  * the WAVs are rendered by UnoDOS's own MIDI synthesiser (dec_midi.c built
    for the host), so even the instrument sounds are ours - there is no
    sampled instrument, soundfont or recording involved.
  * the MP3s are encoded from those WAVs with LAME.

...
# runtime.
#
#   mkuno.py thunks  <syms.txt> <out.s> [x86-64|aarch64]   the thunk assembly
#   mkuno.py convert <in.dll>   <out.uno> [flags]    flatten the linked DLL
#   mkuno.py pyapp   <in.py>    <out.uno> [desc.txt]   wrap Python source
#                                                       (MODF_PYAPP)
#
# Two machines (2026-09-04, cosmo64 M8).  The container is the same on both:
# the loader's job - copy, rebase u64 cells, fill named slots - has nothing
# architecture-specific in it, and PE32+ from llvm-mingw's aarch64 target
# emits the same DIR64 base relocations for absolute pointers (ADRP/ADD pairs
# are PC-relative and need none).  What differs is the thunk (three
# instructions through x16, the intra-procedure-call register the AAPCS64
# reserves for exactly this) and the ABI word: its high byte names the machine
# (0x00 = x86-64, every module before this date; 0xA6 = aarch64), so a loader
# refuses a module built for the other architecture at the `abi` check it
# already had - the alternative was jumping into foreign code.  `convert`
# reads the machine off the PE header; `thunks` has to be told.
#
# .UNO layout (all little-endian):
#   u32 magic 'UNO1'   u16 abi (machine<<8 | version)   u16 flags
...
ABI_VER = 1                 # UNO_ABI_VERSION's low byte
# PE machine -> (name, ABI high byte).  Mirrors uno_app.h's UNO_ABI_VERSION.
MACHINES = {0x8664: ("x86-64", 0x00), 0xAA64: ("aarch64", 0xA6)}
ARCH_BY_NAME = {name: m for m, (name, _) in MACHINES.items()}
ABI   = ABI_VER             # x86-64: the value every module before 2026-09 carries
...
def gen_thunks(syms_path, out_path, arch="x86-64"):
    if arch not in ARCH_BY_NAME:
        sys.exit("mkuno: unknown arch '%s' (one of %s)"
                 % (arch, "/".join(sorted(ARCH_BY_NAME))))
    syms = [s.strip() for s in open(syms_path) if s.strip()]
    for s in syms:
        if len(s) > NAME_MAX:
            sys.exit("mkuno: import name too long (>%d): %s" % (NAME_MAX, s))
    lines = ["/* generated by mkuno.py (%s) - do not edit */" % arch]
    for s in sorted(syms):
        if arch == "aarch64":
            # The slot is data in .unoimp, up to 4 GB away from .text in the
            # linked image, so the reach is ADRP's +/-4 GB page-relative pair,
            # not a 1 MB-limited ADR.  x16 is the AAPCS64 intra-procedure-call
            # scratch register: a veneer may clobber it without the caller
            # noticing, which is what makes this thunk transparent to the
            # function's own argument registers.  The loader writes the slot
            # then syncs the I-cache for the module's pages, so the LDR here
            # reads what the loader stored and the caller's BR lands on kernel
            # code.  `.p2align 3` keeps every slot 8-aligned - the record is 32
            # bytes so the padding is always zero, but the LDR would fault
            # (-mstrict-align) if the section ever started at an odd word.
            code = [
                '\t.p2align 2',
                '\t.globl %s' % s,
                '%s:' % s,
                '\tadrp x16, .Luslot_%s' % s,
                '\tldr  x16, [x16, :lo12:.Luslot_%s]' % s,
                '\tbr   x16',
            ]
            slot = ['\t.section .unoimp,"w"', '\t.p2align 3']
        else:
            code = [
                '\t.globl %s' % s,
                '%s:' % s,
            ...
            slot = ['\t.section .unoimp,"w"']
        lines += ['\t.text'] + code + slot + [
            '\t.asciz "%s"' % s,
    ...
    print("mkuno: %d import thunks (%s) -> %s" % (len(syms), arch, out_path))
...
def convert(dll_path, uno_path, flags=0):
    ...
    if machine not in MACHINES:
        sys.exit("mkuno: PE machine 0x%04x is neither x86-64 nor aarch64" % machine)
    arch, abi_hi = MACHINES[machine]
    abi = (abi_hi << 8) | ABI_VER
    opt = pe + 24
    ...
    print("mkuno: %s  %s entry=0x%x mem=%dK file=%dK relocs=%d imports=%d (%s)%s"
          % (uno_path, arch, entry, size_image // 1024, (48 + len(payload)) // 1024,
             len(relocs), imp_count, " ".join(names) if imp_count else "-",
             "  desc=%dB" % desc_len if desc_len else "  NO DESCRIPTOR"))


if __name__ == "__main__":
    if len(sys.argv) in (4, 5) and sys.argv[1] == "thunks":
        gen_thunks(sys.argv[2], sys.argv[3],
                   sys.argv[4] if len(sys.argv) == 5 else "x86-64")

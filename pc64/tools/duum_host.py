...
class Canvas:
    ...
    # ---- Duum seg rasteriser: the readable mirror of cv_seg_cols in
    # upy_port/mod_uno.c.  DUUM.PY calls this on both sides, so the oracle
    # (tools/duum_verify.py) and the pixel gate (tools/duum_golden.py) check
    # THIS logic on the host, and the C is a transcription of it.  Keep the
    # two in step; duum_golden will catch a drift as changed pixels.
    def duum_frame(self, ctx):
        (self._out, self._ceilc, self._floorc, self._clips, self._masked,
         self._colx, self._dcx, self._dcy, self._skyu, self._sky,
         self._dvsky, self._RW, self._dch, self._hh, self._viewz,
         self._vsc, self._invvsc, self._cceil, self._cfloor,
         self._fbmid, self._fbup, self._fblo) = ctx

    def _d_rect(self, cx, y0, y1, color):
        if y1 < y0: return
        if y0 < 0: y0 = 0
        if y1 > self._dch - 1: y1 = self._dch - 1
        if y1 < y0: return
        x0 = self._colx[cx]; x1 = self._colx[cx + 1]
        self._out.append(('R', x0, y0, x1 - x0, y1 - y0 + 1, color))

    def _d_flat(self, cx, y0, y1, grid, k, lf):
        if y1 < y0: return
        if y0 < 0: y0 = 0
        if y1 > self._dch - 1: y1 = self._dch - 1
        if y1 < y0: return
        x0 = self._colx[cx]; x1 = self._colx[cx + 1]
        self._out.append(('F', x0, x1 - x0, y0, y1 - y0 + 1, grid, k,
                          self._dcx[cx], self._dcy[cx], lf))

    def _d_sky(self, cx, y0, y1):
        if y1 < y0: return
        if y0 < 0: y0 = 0
        if y1 > self._dch - 1: y1 = self._dch - 1
        if y1 < y0: return
        if self._sky is None:
            self._d_rect(cx, y0, y1, _rgb(96, 120, 170))
            return
        x0 = self._colx[cx]; x1 = self._colx[cx + 1]
        self._out.append(('W', x0, x1 - x0, y0, y1 - y0 + 1, self._sky[2],
                          self._sky[0], self._sky[1], self._skyu[cx],
                          y0 * self._dvsky, self._dvsky, 256))

    def _d_wall(self, cx, ytop, ybot, ct, cb, tx, u, v_top, dv, sh, fb):
        ...
        x0 = self._colx[cx]; x1 = self._colx[cx + 1]
        if tx is None:
            self._out.append(('R', x0, y0, x1 - x0, y1 - y0 + 1,
                              _rgb((fb[0]*sh) >> 8, (fb[1]*sh) >> 8,
                                   (fb[2]*sh) >> 8)))
            return
        tw, th, grid = tx
        v0 = int((v_top + (y0 - ytop) * dv) * 256)
        self._out.append(('W', x0, x1 - x0, y0, y1 - y0 + 1, grid, tw, th,
                          int(u), v0, int(dv * 256), sh))

    def seg_cols(self, ix1, ix2, rx1, inv1, k_invd, uz1, k_uz, kfc, kff,
                 kbc, kbf, sflags, lf, wk, mid_t, up_t, lo_t,
                 v_mid, v_up, v_lo, cgrid, fgrid, fc, ff, bc, bf,
                 ldflags, yoff):
        twosided = sflags & 1; bothsky = (sflags >> 1) & 1
        csky = (sflags >> 2) & 1
        hh = self._hh; viewz = self._viewz; vsc = self._vsc; ch = self._dch
        ceilc = self._ceilc; floorc = self._floorc; clips = self._clips
        closed = 0
        th_m = mid_t[1] if mid_t is not None else 0
        for x in range(ix1, ix2 + 1):
            ...
            if twosided:
                ...
                if bothsky and ybc > yfc:
                    yfc = ybc
            if yfc > ct:
                yb = (yfc - 1) if (yfc - 1) < cb else cb
                if csky:
                    self._d_sky(x, ct, yb)
                elif cgrid is not None:
                    self._d_flat(x, ct, yb, cgrid, kfc, lf)
                else:
                    self._d_rect(x, ct, yb, _colrgb(self._cceil, lf * 0.9))
            if yff < cb:
                ya = (yff + 1) if (yff + 1) > ct else ct
                if fgrid is not None:
                    self._d_flat(x, ya, cb, fgrid, kff, lf)
                else:
                    self._d_rect(x, ya, cb, _colrgb(self._cfloor, lf * 0.9))
            if not twosided:
                self._d_wall(x, yfc, yff, ct, cb, mid_t, u, v_mid, dv, shw,
                             self._fbmid)
                ...
                if mid_t is not None:
                    ...
                    if my1 >= my0:
                        x0m = self._colx[x]; x1m = self._colx[x + 1]
                        v0m = int((yoff + (my0 - ytt) * dv) * 256)
                        self._masked.append((dist, ('M', x0m, x1m - x0m, my0,
                                             my1 - my0 + 1, mid_t[2], mid_t[0],
                                             th_m, int(u), v0m,
                                             int(dv * 256), shw)))
                if bc < fc and not bothsky:
                    if up_t is not None:
                        self._d_wall(x, yfc, ybc - 1, ct, newcb, up_t, u, v_up,
                                     dv, shw, self._fbup)
                    else:
                        ya = yfc if yfc > ct else ct
                        yb = (ybc - 1) if (ybc - 1) < newcb else newcb
                        self._d_rect(x, ya, yb, _colrgb(self._cceil, lf * df))
                    if ybc > newct: newct = ybc
                if bf > ff:
                    if lo_t is not None:
                        self._d_wall(x, ybf + 1, yff, newct, cb, lo_t, u, v_lo,
                                     dv, shw, self._fblo)
                    else:
                        ya = (ybf + 1) if (ybf + 1) > newct else newct
                        yb = yff if yff < cb else cb
                        self._d_rect(x, ya, yb, _colrgb(self._cfloor, lf * df))

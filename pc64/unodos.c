...
#define NAPPS      14
#if UNO_COLOR
#define NICONS     14               /* Theme icon is color-only */
...
static const char *kIconNames[NAPPS]  = { "Sys Info", "Clock", "Files", "Notepad", "Music", "Dostris", "OutLast", "Pac-Man", "Tracker", "Paint", "Theme", "Settings", "Network", "Runner3D" };
...
#ifdef UNO_PC64
/* Platform hook: the desktop RESOLUTION changed at runtime (F9 zoom cycle /
   F10 GOP-mode cycle in uefi_main.c). Re-derive the screen rect from the
   shim's updated qd bounds, pull any window whose title bar fell off the
   new (possibly smaller) screen back into reach, and repaint everything. */
void uno_screen_changed(void)
{
    short z;
    gScreen = qd.screenBits.bounds;
    for (z = 0; z < gZCount; z++) {
        UnoWin *w = zwin(z);
        if (w->bounds.left > gScreen.right - 60)
            OffsetRect(&w->bounds, (short)(gScreen.right - 60 - w->bounds.left), 0);
        if (w->bounds.top > gScreen.bottom - 40)
            OffsetRect(&w->bounds, 0, (short)(gScreen.bottom - 40 - w->bounds.top));
    }
    repaint_all();
}
#endif
...
static Boolean fat12_mount(void)
{
    ...
    /* pc64: no block driver yet (NVMe/AHCI are the driver tail), so the
       formatted-in-RAM FAT12 volume is the working PC volume - the same
       fallback the Executor test builds exercise. */
#if defined(UNO_AUTOTEST) || defined(UNO_AUTOTEST_FAT12) || defined(UNO_PC64)
    if (!gFatRam) {
...
static void draw_icon(short i)
{
    ...
    case APP_PAINT:                         /* brush + daub */
        ...
        break;
    case APP_SETTINGS: {                    /* slider panel */
        short s2;
        SetRect(&g, x, iy, x + 18, iy + 17); uno_box(&g, C_CYAN);
        for (s2 = 0; s2 < 3; s2++) {
            short ly = (short)(iy + 4 + s2 * 5);
            SetRect(&g, (short)(x + 3), ly, (short)(x + 15), (short)(ly + 1));
            uno_fill(&g, C_WHITE);
            SetRect(&g, (short)(x + 5 + s2 * 3), (short)(ly - 1),
                        (short)(x + 8 + s2 * 3), (short)(ly + 2));
            uno_fill(&g, C_MAG);
        }
        break;
    }
    case APP_RUNNER: {                      /* 3D corridor: perspective box */
        Rect q;
        SetRect(&g, x, iy, x + 17, iy + 17); uno_box(&g, C_CYAN);
        SetRect(&q, x + 5, iy + 5, x + 12, iy + 12); uno_box(&q, C_WHITE);
#if UNO_COLOR
        RGBForeColor(&kPalette[C_MAG]);
#endif
        MoveTo(x, iy);      LineTo(x + 5, iy + 5);
        MoveTo(x + 17, iy); LineTo(x + 12, iy + 5);
        MoveTo(x, iy + 17); LineTo(x + 5, iy + 12);
        MoveTo(x + 17, iy + 17); LineTo(x + 12, iy + 12);
#if UNO_COLOR
        RGBForeColor(&kBlack);
#endif
        break;
...
static void draw_desktop(void)
{
    ...
    desktop_bg(&gScreen);
#ifdef UNO_PC64
    text_at(gScreen.left + 6, gScreen.top + 14, "UnoDOS PC", C_WHITE, C_BLUE, true);
    text_at(gScreen.left + 6, gScreen.bottom - 8, "UnoDOS/pc64 (UEFI)  v0.2.0", C_WHITE, C_BLUE, true);
#else
...
#endif
#ifdef UNO_PC64
    uno_pc64_init();                /* GOP video + firmware input up before any drawing */
#endif
...
#endif
#ifdef UNO_PC64
        uno_pc64_poll();            /* UEFI keyboard + pointer -> events */
#endif
...
#endif
#ifdef UNO_PC64
        uno_pc64_present();         /* blit the software fb to the GOP framebuffer */
#endif
    }
    return 0;
}

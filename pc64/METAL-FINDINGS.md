...
- **The stick was read afterwards (devbuntu, direct): build `0042` confirmed on
  ...
  - **Consequence: the MBP cannot produce ANY telemetry until this is fixed**: it is untestable, not merely slow, and the watchdog theory above is now only
...
2. **The shipped placeholder `WIFI.CFG` shadowed the real creds.** The driver
   ...
   (`ssid=YourNetwork`) silently won over the flasher-staged `wifi.txt`: the Yoga run tried to join "YourNetwork".

...
class UnoSettings
{
    ...
    public string StressCfg()
    {
        ...
        // Stress Test suite (stress driver). Master OFF -> `nostress` (a real
        // off switch): the driver reads `passes=0`/absent as ENDLESS, so a
        // disabled suite must say so explicitly or it would fuzz forever.
        if (RunStandard) {
            sb.Append("passes=" + (StressPasses < 0 ? 0 : StressPasses) + "\r\n");

; ============================================================================
; UnoDOS/68K games: Dostris (proc 6) + OutLast (proc 7)
; Ports of apps/dostris.asm and apps/outlast.asm. Same piece tables, track
; table, scoring and physics; x86 18.2 Hz tick constants are scaled to the
...
dt_rand7:
        ...
        move.l  d1,d2
        swap    d2
        mulu    #$4E6D,d1           ; low-word multiply pair ~ LCG
        mulu    #$41C6,d2
        swap    d2
        clr.w   d2
        add.l   d2,d1
...
dt_fits:
        ...
        lsl.w   #2,d0
        add.w   d1,d0
        lsl.w   #3,d0               ; (piece*4+rot)*8
        lea     dt_shapes(pc),a0
        lea     (a0,d0.w),a0
        lea     dt_board(pc),a1
        moveq   #0,d7               ; cell index
.cell:  move.b  (a0)+,d4
        ext.w   d4
        add.w   d2,d4               ; c
        move.b  (a0)+,d5
...
; dt_interval -> d0.w = gravity interval in PAL ticks
dt_interval:
        move.w  #18,d0
...
.notfull:
        ...
        tst.w   d7
        beq     .done
        ; score += table[n] * (level+1); lines += n; level = lines/10+1
        lea     vars(pc),a4
        move.w  d7,d0
        add.w   d0,d0
...
dostris_key:
        ...
        move.w  dt_state(pc),d0
        cmp.w   #1,d0
        bne     .nope               ; not playing: nothing else
        cmp.b   #$4F,d2             ; left
        beq     .left
        cmp.b   #$4E,d2             ; right
        beq     .right
...
.band:  move.w  d7,d1
        ...
        move.w  d7,d2
        mulu    #13,d2
        addq.w  #6,d2               ; half width
        move.w  #148,d0
        sub.w   d2,d0
        move.w  #148,d3
        add.w   d3,d2               ; x2 = 148+hw ... careful
        ; recompute cleanly
        move.w  #148,d0
        move.w  d7,d2
        mulu    #13,d2

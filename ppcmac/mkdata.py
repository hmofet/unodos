#!/usr/bin/env python3
"""Generate ppcmac/build/gfx.s from the shared UnoDOS assets (PowerPC Mac / PowerPC).
...
Usage: python ppcmac/mkdata.py   (from the repo root)
"""
import os, re, sys
...
with open(EQU_OUT, "w", newline="\n") as f:
    f.write("# AUTO-GENERATED by ppcmac/mkdata.py - do not edit\n")
...
with open(OUT, "w", newline="\n") as f:
    ...
    for i, p in enumerate(order):
        f.write("    .short " + ",".join("0x%04X" % piece_masks[i*4+r] for r in range(4)) + "\n")
    ...
    for ti, pal in enumerate(THEMES):
        f.write(f"    # theme {ti}\n    .long " + ",".join("0x%08X" % v for v in pal) + "\n")

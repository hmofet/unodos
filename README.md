...
> **browser with a JavaScript interpreter**, and **uno3d** 3D. It boots through UEFI
> and then **leaves it**: `ExitBootServices` is the default, after which the machine
> runs on its own storage (AHCI/NVMe/SDHCI/USB), input (PS2/I2C-HID/USB HID) and
> timers. QEMU+OVMF- and **real-hardware-verified (Lenovo X1 Carbon Gen 8, and a
> ZimaBlade running detached)**. Full detail in **[pc64/README.md](pc64/README.md)**.
...
| [**Nintendo NES**](nes/) | NES (6502/2A03 @ 1.79MHz, PPU 2C02, **2KB RAM**) | Bare-metal iNES NROM-256: PPU tile launcher, patterns in CHR-ROM. The Contract's **`minimal` profile** — no WM, directional nav. Consumes `gen/6502/` + `[world.nes]` (dasm) | **M1–M3 + game + audio — Mesen2-verified + ✅ validated on a real AV Famicom** (2A03 APU audio + `$4016` controller confirmed on hardware — the **first fresh-3.1 port on physical silicon**). Full-screen launcher, pad nav + selection highlight, apps (SysInfo, live Clock, Notepad, Files, Theme-palette, Music on the APU) + **Dostris**. See [nes/README.md](nes/README.md) |
...
| [**Modern PC**](pc64/) | Any x86-64 UEFI PC (~2007+) | Bare-metal **UEFI application** (PE32+ via mingw-w64, no EDK2/gnu-efi): GOP framebuffer + firmware keyboard/pointer to boot, **UEFI-as-BIOS** in the Pi/ppcmac firmware pattern, then `ExitBootServices` by default and on to its own drivers. **First 64-bit x86 world.** Default UI is the **unoui** toolkit as a full themed desktop shell; a `legacy` build keeps the `unodos.c` core + 14 apps. | **QEMU+OVMF-verified + ✅ real hardware (Lenovo X1 Carbon G8; ZimaBlade detached, no firmware).** Default `./build.sh` boots the **unoui shell** (~76 KB, no legacy code): launcher + app windows (Control Panel/Editor/Files/System/Clock/Canvas), 8 live-switchable themes, fill-scaled resolutions, **canvas + fullscreen** toolkit modes, keyboard+pointer nav. `legacy` build adds the family's **first native driver + TCP/IP stack** — Intel **e1000** NIC under a from-scratch **ARP/IP/ICMP/UDP/TCP+DHCP** stack — **TLS 1.2** via BearSSL (pinned key, RDRAND), and **uno3d** 3D (soft rasteriser; Intel-iGPU scaffold). Metal-hardened: eDP SetMode blackout, SMM-trapped debug port, phantom keystrokes, LLP64 DMA truncation; native **I2C-HID trackpad** driver (opt-in). Driver tail now landed: **ExitBootServices** (default), **xHCI** with hubs + **USB mass storage** + **USB HID**, **NVMe/AHCI/SDHCI**. Remaining: real Intel-GPU 3D. See [pc64/README.md](pc64/README.md) |

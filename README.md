...
| [**Modern PC**](pc64/) | Any x86-64 UEFI PC (~2007+) | Bare-metal **UEFI application** (PE32+ via mingw-w64, no EDK2/gnu-efi): GOP framebuffer + firmware keyboard/pointer, boot services alive — **UEFI-as-BIOS**, the Pi/ppcmac firmware pattern on PC silicon. Runs the **full portable C core** (the PS2/DC desktop, WM + all 14 apps) rather than `minimal`. **First 64-bit x86 world.** | **M1–M4 — QEMU+OVMF-verified + ✅ real hardware (Lenovo X1 Carbon G8).** Desktop/WM/apps, Notepad **Ctrl-S save**, **Dostris**, PC-speaker audio; a **Settings** resolution picker (runtime fb resize, 640×480…1920×1200); the family's **first native driver + TCP/IP stack** — an Intel **e1000** NIC under a from-scratch **ARP/IP/ICMP/UDP/TCP + DHCP** stack (DHCP + ping + UDP + TCP all verified vs SLIRP); and **uno3d** ported — the write-once Runner 3D game (soft rasteriser into GOP; Intel-iGPU backend scaffolded). Metal-hardened: eDP SetMode blackout, SMM-trapped debug port, phantom keystrokes, LLP64 DMA truncation. Driver tail: ExitBootServices, xHCI HID, NVMe/AHCI, real Intel-GPU 3D. See [pc64/README.md](pc64/README.md) |

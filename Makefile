...
$(TRACKER_BIN): $(APPS_DIR)/tracker.asm | $(BUILD_DIR)
	$(NASM) -f bin -o $@ $<

$(PAINT_BIN): $(APPS_DIR)/paint.asm | $(BUILD_DIR)
	$(NASM) -f bin -o $@ $<
...
apps: $(CLOCK_BIN) $(LAUNCHER_BIN) $(BROWSER_BIN) $(MOUSE_TEST_BIN) $(MUSIC_BIN) $(MKBOOT_BIN) $(SETTINGS_BIN) $(TETRIS_BIN) $(TETRISV_BIN) $(NOTEPAD_BIN) $(SYSINFO_BIN) $(OUTLAST_BIN) $(OUTLASTV_BIN) $(PACMAN_BIN) $(PACMANV_BIN) $(TRACKER_BIN) $(PAINT_BIN)
...
build/launcher-floppy.img: $(LAUNCHER_BIN) $(CLOCK_BIN) $(BROWSER_BIN) $(MOUSE_TEST_BIN) $(MUSIC_BIN) $(MKBOOT_BIN) $(SETTINGS_BIN) $(TETRIS_BIN) $(TETRISV_BIN) $(NOTEPAD_BIN) $(SYSINFO_BIN) $(OUTLAST_BIN) $(OUTLASTV_BIN) $(PACMAN_BIN) $(PACMANV_BIN) $(TRACKER_BIN) $(PAINT_BIN)
	@echo "Creating launcher floppy image..."
	python3 tools/create_app_test.py $@ $(LAUNCHER_BIN) LAUNCHER.BIN $(SYSINFO_BIN) SYSINFO.BIN $(CLOCK_BIN) CLOCK.BIN $(BROWSER_BIN) BROWSER.BIN $(MUSIC_BIN) MUSIC.BIN $(SETTINGS_BIN) SETTINGS.BIN $(TETRIS_BIN) TETRIS.BIN $(TETRISV_BIN) TETRISV.BIN $(NOTEPAD_BIN) TEXT.BIN $(OUTLAST_BIN) OUTLAST.BIN $(OUTLASTV_BIN) OUTLASTV.BIN $(PACMAN_BIN) PACMAN.BIN $(PACMANV_BIN) PACMANV.BIN $(TRACKER_BIN) TRACKER.BIN $(PAINT_BIN) PAINT.BIN

; ============================================================================
; UnoDOS/MacPlus Paint (proc 8) - the shared MacPaint-style editor on the
; 1-bit screen. Port of amiga/paint.i with the platform-gamut rule applied:
; the Mac Plus gamut is the kernel's four row-dither inks (white, 25%, 50%,
; black), so the pen strip is 4 large swatches; the r/g/b channel tuning
; (12-bit OCS color) has no 1-bit analog and is dropped. Canvas background
; is WHITE (ink 0) with black (3) the default pen - proper MacPaint.
;
; Tool palette down the left (pencil, brush, eraser, line, rect, frect,
; oval, foval, flood fill, spray), pen strip along the bottom, drag-to-draw
; canvas with a black rubber-band preview for the shape tools. The drag
; loops poll the ISR-maintained mouse state synchronously; the software
; cursor stays hidden during a stroke (cur_erase ran before handle_clicks),
; which is exactly what MacPaint did.
;
;   1..9,0    select tool      [ / ]  previous / next ink
;   n         clear canvas     s / l  save / load PAINT.UNO (boot floppy)
; ============================================================================

PT_W        equ 256
PT_H        equ 140
PT_TOOLS    equ 10
PT_BGPEN    equ 0                   ; canvas background: white
PT_NPENS    equ 4                   ; the 1-bit dither gamut
...
.tool:  cmp.w   #PT_TOOLS,d7
        ...
        moveq   #12,d2
        moveq   #12,d3
        moveq   #0,d4               ; white cell
        cmp.w   pt_tool(pc),d7
        bne     .tbg
        moveq   #3,d4               ; selected = black cell
.tbg:   bsr     fill_rect
        moveq   #12,d2
        moveq   #12,d3
        bsr     rect_outline_fg     ; cell border
...
.dig:   add.w   #'0',d2
        ...
        move.b  d2,(a0)
        clr.b   1(a0)
        moveq   #3,d2               ; black digit...
        moveq   #-1,d3
        cmp.w   pt_tool(pc),d7
        bne     .gl
        moveq   #0,d2               ; ...white on the selected cell
        moveq   #3,d3
.gl:    bsr     draw_string_bg      ; (bg -1 = transparent path)
        movem.l (sp)+,d0-d1
        addq.w  #1,d7
        bra     .tool
.pens:  ; ink strip: 4 swatches of 16x10 along the bottom
        moveq   #0,d7
.pen:   cmp.w   #PT_NPENS,d7
        ...
        moveq   #15,d2
        moveq   #9,d3
        bsr     rect_outline_fg     ; swatch border (white needs one)
        cmp.w   pt_pen(pc),d7
        bne     .npen
        ; selection: black tick under the active ink
        addq.w  #6,d0
        add.w   #11,d1
        moveq   #4,d2
        moveq   #2,d3
...
; pt_pen_rect - d7 = ink -> d0/d1 = swatch screen pos
pt_pen_rect:
        move.w  d7,d0
        mulu    #18,d0
        add.w   WX(a2),d0
...
; canvas (mouse state is ISR-updated).
paint_click:
        movem.l d0-d7/a0-a1/a4,-(sp)
        lea     vars(pc),a4
...
; pt_drag_shape - d0/d1 = anchor: rubber-band (black frame preview,
...
; pt_band_draw / pt_band_undraw - black preview frame between the
...
; pt_band_edges_pen - draw the 4 frame edges in black via fill_rect
pt_band_edges_pen:
        movem.l d0-d7,-(sp)
        move.w  pt_px0(pc),d0
        move.w  pt_py0(pc),d1
...
; pt_save / pt_load - PAINT.UNO on the boot floppy's FAT12 volume
; (lazy-mounts like Files does)
pt_save:

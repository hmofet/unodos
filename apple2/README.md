...
- **Catalog = rel sector 0**, cached in RAM at `CATBUF` ($6400) and flushed
  ...
  - `16..255` 15 directory entries x 16 bytes: `name[12]` (NUL-padded),
    ...
    `FS_SECTORS=240 < 256`).
- A rel sector `r` maps to `(track, logical) = (FS_TRACK + (r>>4), r&$0F)`: `FS_SECTORS=240` is a multiple of 16, so this is exact (no remainder
...
- **Notepad**: 2 KB text buffer (`NOTEBUF`, $6500-$6CFF). Typing inserts at
  ...
  Return inserts a newline, ESC closes. **Ctrl-S saves** (`$93`: Ctrl-letters arrive as codes < `$20 | $80` on every Apple II keyboard).

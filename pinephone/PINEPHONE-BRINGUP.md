...
`clk_init` is split into four per-block helpers, `tcon0_clk_init`, `dsi_bus_clk_on`, `dphy_clk_on`, `de_clk_init`: each called at the p-boot-equivalent point in `panel_init` (so PLL_DE/DE clock now come up **last**, after the
...
`panel_dsi_only`, which re-inits ONLY the DSI link, `dsi_host_init` + `dphy_init` + panel reset/DCS + `dsi_start`: on top of p-boot's clocks AND TCON0) ALSO went black. Trace: panel STILL answers DCS after our re-init
...
Executed the new strategy. **#3 (re-derive the computed timing values) is COMPLETE and ELIMINATED**: done analytically, no hardware needed. p-boot *computes* four timing values from the XBD599 panel
...
`shots/trace-pboot-tcon0full.log`. **Full TCON0 diff (138 regs each): 2 differences, BOTH live status**: `GINT0` (frame-done latch, sampled mid-flight) and `TRI1` hi-half (the running block counter). **Every
...
`dphy_init` matches its values + order; our only delta is `delay_ms(1)` where p-boot uses `udelay(1-5)`: i.e. ~1000× *longer* settling, which is safe (not too short). So D-PHY is faithful, poll included (there is
none).

...
| **M7** 2026-08-06, VERIFIED against a real server | progressive render during fetch, style sharing, keep-alive + parallel TLS, perf pass | LANDED AND TESTED (`tools/netverify_urc.py` + `nettest_server.py`, a controlled server the QEMU guest reaches at 10.0.2.2): **response framing** (Content-Length + chunked - a chunked page renders its decoded text), **progressive render** (screenshot 1.8 s into a deliberately-paused response shows the heading and first lines already painted), **style sharing**. **keep-alive is DISABLED**: against a real server it did not reuse anything AND the browser hung after the second request. One cause was found and fixed (a connection that could not be kept was never closed, and pc64 has a single TCP slot); the hang had a second, larger cause - the progressive callback repainted from inside the receive loop and re-entered the fetch path, starting a NESTED request on that same slot. That is fixed with a re-entrancy guard, but reuse stays off until it can be re-tested deliberately. **parallel TLS** needs per-socket TLS from unonet and has no path until that exists. |

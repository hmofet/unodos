...
| **M7** NOT STARTED | progressive render during fetch, style sharing, keep-alive + parallel TLS, perf pass | Untouched as of 2026-08-06. Notes for whoever takes it: **keep-alive is not a small change** - the receive loop currently reads until the server closes, so a persistent connection needs Content-Length and chunked-transfer parsing first, and getting that wrong hangs the browser on every response. **Progressive render** needs the fetch to hand bytes to `uw_parse_feed` as they arrive (the parser is already a push interface and takes partial input, so the work is in pc64_http's receive loop, not in unoweb). **Style sharing** is self-contained: elements that cascade identically can share one computed style, and `uw_style` is already a fixed struct compared by memcmp. The response cache landed in the chrome work already covers the repeat-visit half of M7's motivation. |

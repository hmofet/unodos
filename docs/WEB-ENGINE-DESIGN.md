...
| **M7** DONE 2026-08-06 | progressive render during fetch, style sharing, keep-alive + parallel TLS, perf pass | LANDED: **response framing** (Content-Length + chunked), **keep-alive** (one connection across a page's subresources, retry-once on a dropped reuse), **progressive render** (body offered every ~6 KB, painted mid-fetch), **style sharing** (same parent node + same tag + same full attribute set; switched OFF wholesale for any sheet containing a positional selector, since position is what such a key cannot see - `uw_share_hits()` proves it fires rather than silently never engaging). NOT DONE: **parallel TLS**, which needs per-socket TLS from unonet and has no path until that exists. The network three are **UNVERIFIED against a live server** - the gate's network area runs on a null NIC. |

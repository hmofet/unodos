...
| **M7** PARTIAL 2026-08-06 | progressive render during fetch, style sharing, keep-alive + parallel TLS, perf pass | LANDED: **response framing** (Content-Length + chunked) and **keep-alive** - one connection held across a page's subresources, HTTP/1.1, with a retry-once when a reused connection turns out to have been dropped. **Keep-alive is UNVERIFIED against a live server** - it builds and the suite is green, but the gate's network area runs on a null NIC, so the first real page load is the real test. Its failure mode is bounded: a wrong keep decision costs the existing ~3 s idle timeout, not a hang. NOT DONE: **progressive render** (feed `uw_parse_feed` from the receive loop; the parser already takes partial input), **style sharing** - ATTEMPTED AND REVERTED, the key (tag/class/id/style/parent style) computed a WRONG COLOUR for an `li` in the golden suite; find what it misses before retrying. |

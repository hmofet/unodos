...
| **M7** DONE 2026-08-06, verified against a real server | progressive render during fetch, style sharing, keep-alive + parallel TLS, perf pass | ALL LANDED AND MEASURED (`tools/netverify_urc.py` + `nettest_server.py`): **framing** (Content-Length + chunked), **keep-alive** (a page and its three stylesheets on ONE connection, counted server-side), **progressive render** (a screenshot 1.8 s into a deliberately-paused response shows the heading and first lines already painted), **style sharing**. Keep-alive's earlier failure was ROOT-CAUSED: it was never independently broken - both the absent reuse and the browser hang were collateral damage from the progressive-render re-entrancy bug (a repaint from inside the receive loop starting a nested request on the single TCP slot), plus a connection that could not be kept never being closed. NOT DONE: **parallel TLS**, which needs per-socket TLS from unonet and has no path until that exists. |

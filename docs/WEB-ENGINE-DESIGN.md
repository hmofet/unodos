...
| **M7** PARTIAL 2026-08-06 | progressive render during fetch, style sharing, keep-alive + parallel TLS, perf pass | LANDED: **response framing** - Content-Length and chunked, so a response ends when its last byte lands instead of when the server hangs up (or a 3 s idle timeout fires). That is both a latency win now and the hard prerequisite for keep-alive. NOT DONE: **keep-alive** itself (the framing it needed now exists, so this is the next step), **progressive render** (feed `uw_parse_feed` from the receive loop - the parser already takes partial input), **style sharing** - ATTEMPTED AND REVERTED: caching a cascade result keyed on tag/class/id/style/parent style computed a WRONG COLOUR for an `li` in the golden suite, so the key is missing something the cascade depends on. A slower page beats a wrong one; find what the key misses before retrying. |

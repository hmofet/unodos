...
| ↳ A6c | **USERSPACE 2026-08-06: `Run /init as init process`.** A 1.1 MB busybox initramfs loads into the carve at GPA `0x20000000` and is handed over through `ramdisk_image`/`ramdisk_size`; Linux unpacks it and execs `/init`. Needed **the SYSCALL MSRs written through to the real machine** (LSTAR/STAR/SFMASK/KERNEL_GS_BASE were held in a variable, which works until userspace executes its first `syscall` - SYSCALL reads LSTAR out of the CPU, not out of us), and a `/dev/console` node, without which init gets no stdin and a shell reading EOF exits. **Open: `/init` exits with code 0 printing nothing**, so it fails before its first echo - likeliest a redirection that fails silently in a non-interactive shell. Next: an init script that cannot fail quietly. | 1 |

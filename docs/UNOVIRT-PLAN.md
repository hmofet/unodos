...
| ↳ A6a | **IN PROGRESS 2026-08-06:** the loader works and a real kernel runs. 16.8 MB Ubuntu bzImage read off the filesystem into the carve, setup header copied verbatim, e820 written with the loader's tables in a reserved run, entered at the 64-bit entry with the zero page in RSI. Three faults so far, each further in: a triple fault at entry+0x7e (no IDT yet - fixed by trapping #UD/#GP/#PF), then `#GP(0)` there once it was visible (**VMX forces CR4.VMXE and Linux clears it - a kernel cannot boot without CR shadow registers**), and now `#PF(0)` at guest `0x40e` with RIP `0x543fa80`, which is past the decompressor's self-relocation. **Nothing printed yet**, so the first question is whether it accepted the command line at all. | 1 |

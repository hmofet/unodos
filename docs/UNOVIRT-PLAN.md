...
| **A1** | SVME/VMXON, a vCPU context, the `cpuid` round trip, and the crasher guest | `guest cpuid round trip -> … OK`; then a deliberate fault yields `GF001.TXT` and the desktop keeps running | 2..3 (spent: 1) |
...
| ↳ A1b | The VMX backend, and the first run at **one** level of nesting: devbuntu is bare metal, Intel, `kvm_intel nested=Y`. It settles A1a and exercises the arm both metal boxes need. | 2..3 |

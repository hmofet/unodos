...
| ↳ A6b | What a shell needs from here: somewhere to run that is not a boot-time selftest (`uno_vmm_tick`, which A3 built), a LAPIC and a timer (`nolapic no_timer_check` papers over their absence, and a kernel with no timer cannot schedule), an initramfs, and A5's virtio-console as the real console. | 2..3 |

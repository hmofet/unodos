...
| **A0** | `uno_hv_t` seam, `uno_vmm_eligible()` with attributed blockers, the `HV :` banner, Settings row | on the ZimaBlade and the X1: correct capability line; on a machine with VMX disabled in firmware, the named blocker and no fault | 1..2 |
| **A1** | VMXON/SVME, a vCPU, the `cpuid` round trip, and the crasher guest | `guest cpuid round trip -> … OK`; then a deliberate fault yields `GF001.TXT` and the desktop keeps running | 2..3 |
| **A2** | EPT/NPT, `uno_vmm_reserve()`, the carve, WB memory type | a guest stores at GPA `0x100000` and the value is read back at the carve's HPA; MTRR check in the same line | 2 |
| **A3** | The frame-loop slice: preemption timer, budget, heartbeat, resumable context | a guest spinning in an infinite loop while the desktop drags a window; watchdog never fires, hitch counter clean over 10 minutes | 2 |
| **A4** | Guest LAPIC and timer, exit decode into `uno_vmexit`, MSR and CR intercepts | guest reads a monotonic clock and takes its own timer interrupt | 2..3 |
| **A5** | virtio-mmio transport + console, with the bounds-checked guest-memory seam written first | guest prints through a virtqueue; used ring agrees on the byte count; a malicious descriptor cycle is refused, not hung on | 2..3 |
| **A6** | Linux boot protocol: bzImage, `boot_params`, initramfs | Linux boots to a shell on virtio-console under UnoDOS, on metal | 3..4 |

...
| ↳ A6c | An initramfs, which is now the only thing between here and userspace and is supply rather than mechanism (`ramdisk_image`/`ramdisk_size` are wired and zero). Then a LAPIC and timer, and A5's virtio-console as the real console. | 2..3 |

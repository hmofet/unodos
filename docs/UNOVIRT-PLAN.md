...
> **Status: Track A phases A0..A8 are BUILT and pass on VMX** (nested-KVM
> harness boxes; devbuntu, now also leviathan); a real kernel boots under
> UnoDOS, its shell reads a command and answers, it mounts an ext4 disk out
> of a file, and it is bridged to the real wire. **M5 is met, 2026-08-23: an
> address typed on the HOST navigates Chromium inside the guest**, in a
> native unoui window, on a wlroots kiosk running on the guest's own
> framebuffer. Everything after it - and all
> of Tracks B and C - is still plan. The phase table in §7 is the authority
> on which is which, and `pc64/UNOVIRT.md` carries the findings.
...
| ~~**A6**~~ | ~~Linux boot protocol: bzImage, `boot_params`, initramfs~~ | **MET on VMX 2026-08-07**, on the 8250 rather than virtio-console (the same claim, through the device a kernel can talk to in its first millisecond): `~ # echo UNODOS-GUEST-SHELL-OK` → `UNODOS-GUEST-SHELL-OK`, then `uname -a` and `ls /bin`, all answered. **The reply is the criterion** - kernel output only ever proved a kernel runs, while an answer proves the guest read a line we sent, ran it and replied. | 3..4 (spent: 6) |
...
| ↳ A6e | **THE SHELL ANSWERS 2026-08-07.** The 8259 pair was the missing piece and the smallest of five. The other four are all cases where the CONSERVATIVE choice is the broken one: **the idle loop is `sti; hlt`**, so the HLT exit carries an interrupt shadow and refusing to inject into it means never waking the guest whose HLT you just consumed (injections froze at 76 while the PIC showed both lines up); **delivery must not depend on sampling a lucky moment**, so interrupt-window exiting rather than a look-and-hope; **a periodic device must count GUEST time** (a quarter-core guest driven by a wall-clock tick gets four periods per period of service and livelocks in its own timer handler) **while a device the guest READS must count the wall**; and **transmit-empty is a latch that reading IIR clears, not a level** - modelled as a level it can never be quieted, which wedged the shell at the exact moment `ls` first needed interrupt-driven output. Plus DLAB, whose absence put the baud divisor's high byte over the IER. | 1..2 (spent: 1) |
| ↳ A6f | **The trap that cost more than any bug here, recorded because it is invisible:** the shipped DEBUG.CFG carries `passes=3`, so the machine POWERS ITSELF OFF about nineteen seconds in - and a guest running at 4 ms a frame gets about a tenth of a second of CPU out of that, not enough to finish decompressing itself. It presents as a kernel that stops after one line, which is indistinguishable from a hang and was chased as one twice. **`noshutdown` belongs in DEBUG.CFG beside `vm-selftest`**; `tools/hv_remote.py` now carries the recipe and prints the guest's own output. The tell that was there all along: the guest's exits were nearly all preemption-timer exits, which means busy, not stuck. | 0 |
| ~~**M3**~~ | The guest bridged to the REAL wire, and a browser on it | **MET 2026-08-21.** The guest DHCPs its own lease off the wire (10.0.2.16 beside the host's .15), resolves names, and completes a TLS handshake against real roots; **Chromium renders `unodos.arinbakht.com` in a native UnoDOS window**. R3 is answered more cheaply than either plan option: the guest keeps its own MAC, because every NIC driver here already runs promiscuous, so no NAT and no IP demux is needed. Frames for the guest's MAC are taken exclusively; broadcast and multicast go to both stacks. | 1 |
| ~~**M4**~~ | A browser appliance: a real display server on the guest's framebuffer | **MET 2026-08-22.** Xorg segfaults on this guest's framebuffer (simpledrm, no GPU, no render node) under modesetting, under modesetting with acceleration off, and differently under fbdev, and its fatal backtrace never survives to its own log. A **wlroots kiosk (cage) with the pixman software renderer** needs no GPU, reads evdev itself, and gives Chromium the whole screen focused. **It did not actually run until M5**: `cage -D` was added as a debug flag, cage 0.1.5 has no such flag, and the appliance fell silently through to its X fallback on every boot in between - which works under plain QEMU, so nothing ever looked wrong. | 1 |
| ~~**M5**~~ | The browser DRIVEN from the host: an address typed through the window | **MET 2026-08-23.** `Ctrl+L` plus `example.net`, typed a letter at a time through the Appliances window, navigates the guest's Chromium (`shots/browser_host_typed.png`). M4's "plain letters do not reach the browser" was not a keyboard fault at all: the compositor was not running, the screenshot that "proved" modifiers arrived was a fast-loop capture, and the guest's own `irq1=` counter read CPU0 alone and so reported zero while every keystroke arrived. `libinput debug-events` on the keyboard node now names each key the guest sees, and the fast loop reproduces the hypervisor's byte stream **without the hypervisor** through i8042 command 0xD2. **Still open: Chromium aborts on `FastDtoa: v > 0`** - Alpine links it against the system double-conversion, whose assertions are live - so it is restarted inside the compositor and the harness retries. | 1 |
| **A7** | virtio-blk over a unofs image file, virtio-net bridged at `uno_nic_t` | **HALF MET 2026-08-07 (A7a, blk).** `EXT4-fs (vda): mounted filesystem ... ro` then `cat /mnt/HELLO` -> `UNODOS-GUEST-DISK-OK`: the guest mounted a filesystem out of an ordinary file on a UnoDOS volume. Read-only, because unofs and native FAT both read from an offset but write only WHOLE FILES - a request for `uno_fs_write_at` is filed. The transport went from one device/one queue to several/two, and the chain walk now carries each buffer's DIRECTION. virtio-net (A7b) is the remaining half. | 4..5 (spent: 1) |
| &#8627; A7a | **The bug was not in the new code.** `gpr()` had **RSI and RDI mapped to each other's slots**, latent since A5 because every hand-written test guest uses rax/rbx/rcx/rdx and guest port I/O goes through rax. virtio-mmio writes its status byte from ESI, so every status write arrived as garbage and the driver reported `device refuses features: 0` - a conclusion about features that had nothing to do with features. Recording the register TRAFFIC, and reading the driver's side of the conversation instead of its verdict, is what found it. Also: a console's **queue 0 is RECEIVE**, and completing those buffers empty is an interrupt storm that stalls the boot; and `/dev/vda` needs `mount -t devtmpfs`, because this initramfs carries only `/dev/console`. | 1 |
| &#8627; A7-harness | **The harness cost more than the phase.** The "occasional empty capture" was never flakiness: a timed-out run leaves QEMU alive holding the image and log locks. QEMU's stderr went to `/dev/null`, hiding it. `pkill -f qemu-system-x86_64` matched the run script ITSELF (the script arrives as the remote shell's command line and names QEMU further down), so the run killed itself before starting - ssh 255, nothing on either stream. And the image check scanned only the first 4 MB, so once a 17 MB kernel was staged beside DEBUG.CFG it warned that an armed image was not armed. The run script now ships as a FILE, waits for stragglers, and reports QEMU's exit; `tools/vm_stage.py` does the staging. | 0 |
| ~~**A8**~~ | Guest framebuffer + i8042 input + **L0**: whole guest display in one unoui window | **MET 2026-08-21 on the KVM harness**: `shots/vmdisp_display_typed.png` shows the guest's fbcon inside the Appliances window, with `echo UNODOS-DISPLAY-OK` typed THROUGH the window (URC key -> shell -> emulated i8042 -> guest VT) and answered on the guest's own framebuffer.  The reply is the criterion, as ever: pixels alone prove a blit, an answer proves the whole loop. | 2 (spent: 1) |
| ↳ A8a | **DONE 2026-08-21.** The display is `boot_params.screen_info` (VIDEO_TYPE_EFI, XRGB8888, top of the carve, e820-reserved) rather than the custom frames device the table above sketched: a stock kernel drives a linear framebuffer with NO driver from us (sysfb -> simpledrm/efifb), where a custom device needs a guest agent that is B1's work. The custom damage-rect device still comes with B1, for L1. Input is an emulated i8042 (PS/2 set-1 keyboard + IntelliMouse aux, slave PIC cascade wired for IRQ12), NOT virtio-input: the virtio-input driver reads whole capability bitmaps in one config access, virtio-mmio v2's `vm_get` BUG()s on any length outside {1,2,4,8}, and every distro ships it `=m` besides. The Appliances app gained a Display view (blit + swizzle, keys and pointer forwarded; F12 leaves). The appliance payload is now built by `guest/appliance/` (custom 6.12 kernel, everything =y; busybox initramfs with a shell on ttyS0 AND tty1; Alpine+Xorg+Chromium rootfs for the browser milestone). **Finding that cost the session's middle: a PARKED GUEST STILL EXPERIENCES TIME.** The PIT counts guest cycles (A6e's livelock fix), but a halted guest accrues none - so the one-shot timer a NO_HZ kernel arms before idling never fired, and the boot slept forever the first time its boot thread blocked, at `default_idle+0xe`. Found by the quiet-guest dump (ten silent seconds -> RIP + register file + the bytes under RIP, resolved against System.map), which turned three wrong theories (console handover, a panic reboot loop, an MSR poll) into one right one. While parked, guest time now advances at wall rate. | 1.. |

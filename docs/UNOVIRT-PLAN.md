...
| ↳ A6b | **IN PROGRESS 2026-08-06: 128 lines deep.** The kernel now runs from `uno_vmm_tick` - a 4 ms slice per frame, for as long as it takes - rather than inside a selftest. Three findings: **one VMCS means one guest** (arming A3's spinner silently replaced a booting Linux with `jmp $`); **a kernel calibrates its clock against an 8254**, so it sat on port 0x42 forever until channel 2 counted, driven by the real TSC so the ratio survives slicing (24 lines -> 54); and **INVPCID raises #UD in a guest unless a secondary control enables it** while CPUID says it is available, which Linux believes in `native_flush_tlb_global` (54 -> 128). Left: the next unhandled exit, a LAPIC and timer, an initramfs, virtio-console. | 1 |

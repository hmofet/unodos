...
| ↳ A6d | **The UART receive path**, and it is the last thing between here and a shell: an RX FIFO with LSR bit 0, UnoDOS keystrokes pushed in from the frame loop that already owns the keyboard and already calls `uno_vmm_tick`, and the transport interrupt injected on arrival (A4 proved that path and it has been idle since). | 1..2 |
